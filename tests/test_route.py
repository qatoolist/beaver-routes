...
class TestRoute:
    ...
    def test_for_scenario_route(self, verify_route):
        ...
        assert id(invalid_key_route) == id(verify_route), "scenario route and calling route should be same"
        assert verify_route.scenario_method, "scenario method not set to calling route"
        assert verify_route.current_scenario_name == scenario_name, "current scenario name did not match"
    ...
    def test_for_scenario_with_group_route(self):
        ...
        scenario_route = route.for_scenario(scenario_name, group_name=scenario_group_name)

        assert id(scenario_route) == id(route), "scenario route and calling route should be same"
        assert route.scenario_method, "scenario method not set to calling route"
        assert route.current_scenario_name == scenario_name, "current scenario name did not match"
        assert route.current_scenario_group == scenario_group_name, "current scenario group name did not match"
    ...
    def test_argument_key_merge(self, mock_request, verify_route):
        ...
        response = verify_route.for_scenario("valid_key").get(**{"params": {"key": "invalid"}})
        assert response.status_code == 400
        assert response.json() == {"message": "bad key"}
    ...
    def test_argument_dictionary_merge(self, mock_request, verify_route):
        ...
        response = verify_route.for_scenario("valid_key").get(**{"params": {"additional_key": "invalid"}})
        assert response.status_code == 200
        assert response.json() == {"message": "valid key"}
    ...
    def test_missing_endpoint_with_url(self, mock_request, verify_route):
        ...
        response = verify_route.for_scenario("valid_key").get(**{"url": "https://api.test.com/verify"})
    ...
    def test_default_validator(self, validator, pong_route):
        ...
        kwargs = {"expected__status_code": 201, "test_kw1": "test_kv1", "test_kw2": "test_kv2"}
    ...
    def test_custom_validator(self, validator, pong_route):
        args = ("test_arg1", "test_arg2")
        kwargs = {"expected__status_code": 404, "test_kw1": "test_kv1", "test_kw2": "test_kv2"}
        pong_route.validate(*args, name="pong_validator", *kwargs)
        validator.assert_called_once_with(*args, *kwargs)

    def test_no_validator(self, pong_route):
        with pytest.raises(NotImplementedError, match="validator should defined in derived route"):
            pong_route.validate()

    def test_invalid_validator(self, pong_route):
        validator_name = "ping_validator"
        expected_error = f"Validation attribute '{validator_name}' not found in derived route"
    ...
    def test_url_placeholders(self, mock_request, verify_route):
        ...
        assert mock_request.call_args.kwargs["url"] == f"{VerifyRoute.BASE_URL}/users/user123"

    @patch('requests.request', side_effect=mock_request_side_effect)
    def test_merge_replace(self, mock_request, pong_route):
        pong_route.route_attributes = {"json": {"data1": "value1", "nested_key1": {"data1": "value1"}}}
        pong_route.method_attributes = {
            "json": {"data2": "value2", "nested_key1": {"_merge_strategy": "replace", "nest_key": "nest_value"}}
        }
    ...
    def test_merge_remove(self, mock_request, pong_route):
        pong_route.route_attributes = {"json": {"data1": "value1", "nested_key1": {"data1": "value1"}}}
        pong_route.method_attributes = {"json": {"data2": "value2", "nested_key1": {"_merge_strategy": "remove"}}}
        pong_route.scenario_attributes = {"json": {"data3": "value3"}}
    ...
    def test_nested_multi_merge(self, mock_request, pong_route):
        pong_route.route_attributes = {"json": {"data": "value", "json_key1": {"data1": "value1"}}}
        pong_route.method_attributes = {
            "json": {
                "json_key1": {"d2": "value"},
                "nest2": {
        ...
        pong_route.scenario_attributes = {
            "json": {
                "json_key1": {"d3": "value"},
                "nest2": {
                    "nest3": {"json_nested_key3": {"_merge_strategy": "replace", "k2": "v2"}},
                    "nest4": {"json_nested_key4": {"_merge_strategy": "deep_merge", "k2": "v2"}},
    ...
    def test_invalid_merge_strategy(self, mock_request, pong_route):
        pong_route.route_attributes = {"params": {"data1": "value1", "nested_key1": {"data1": "value1"}}}
        pong_route.method_attributes = {"params": {"data2": "value2", "nested_key1": {"_merge_strategy": "pop"}}}
    ...
    def test_with_scenario_group(self, mock_request):
        route = GroupPongRoute()
        scenario_route = route.for_scenario("group_scenario", group_name="pong_scenario")
        scenario_route.get()
        expected_merged_args = {
            "method": "get",
            "params": {
        ...
        assert route.scenario_method.__self__.route == route, "scenario group should get route reference assigned"
        assert mock_request.call_args.kwargs == expected_merged_args, "merged request args does not match"
