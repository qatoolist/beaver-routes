...
def mock_httpx_client(monkeypatch)-> None:
    ...
    async def mock_async_request(self, method, url, **kwargs) -> MockResponse:
        return MockResponse()

    def mock_request(self, method, url, **kwargs) -> MockResponse:
        return MockResponse()
...
class TestBaseRoute:
    ...
    async def test_meta_exception(self, monkeypatch) -> None:
        def mock_to_httpx_args_raise(meta, method) -> None:
